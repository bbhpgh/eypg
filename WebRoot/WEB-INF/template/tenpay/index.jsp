                    ...
                    <dl>
                        ...
                        <dd>
                            ...
                            <p>您现在可以 <a class="blue"
                                        href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/user/UserBuyList.html">查看<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>
                                记录</a> 或 <a class="blue"
                                            href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>">继续购物</a>
                            </p>
                            <p>总共成功<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>
                                ${fn:length(successCartList)}件商品，信息如下：</p></dd>
                    </dl>
                </div>
                <div class="wait_listCon">
                    <ul>
                        <li class="wait_Ltit"><span class="wait_Cw01">购买时间</span><span
                                class="wait_Cw02">商品名称</span><span
                                class="wait_Cw03"><%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>人次</span><span
                                class="wait_Cw04"><%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>编码</span>
                        </li>
                        <s:iterator value="successCartList" var="successCart">
                            <li class="wait_Lodity"><span class="wait_Cw01">${successCart.buyDate}</span><span
                                    class="wait_Cw02"><a class="blue"
                                                         title="(第${successCart.productPeriod }期)${successCart.productName }"
                                                         href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${successCart.productId }.html">(第${successCart.productPeriod }期)${successCart.productName }</a></span><span
                                    class="wait_Cw03">${successCart.buyCount}</span><span class="wait_Cw04"><a
                                    href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/user/UserBuyDetail-${successCart.productId }.html"
                                    target="_blank">查看详情</a></span></li>
                        </s:iterator>
                    </ul>
                </div>
            </c:when>
            <c:otherwise>
                <div class="wait_list_tips ">
                    <dl>
                        ...
                        <dd>
                            <h2 class="orange Fb">哎呀！<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>
                                失败了。已经被人抢拍了吧。</h2>
                            <p>您现在可以 <a class="blue"
                                        href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/mycart/index.html">查看购物车</a>
                                或 <a class="blue"
