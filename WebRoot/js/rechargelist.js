$(document).ready(function () {
    ...
    function pageselectCallback(pageNo) {
        var url = "/user/ConsumeListByDeltaAjaxPage.action?pageNo=" + pageNo + "&selectTime=" + R.val() + "&userId=" + userId + "&startDate=" + startDate + "&endDate=" + endDate;
            ...
            success: function (data) {
                ...
                if (resultCount > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        var W = "<ul class=\"content\" >";
                        W += "<li class=\"time\">" + data[i].date + "</li>";
                        W += "<li class=\"bank\">" + "网银支付" + "</li>";
                        W += "<li class=\"much\">\uffe5" + data[i].money + "</li>";
                        W += "</ul>";
                        $(W).appendTo("#consumer_records_list");
                    }
                    $("#pageLoading").hide();
                } else {
                    $("<ul><li class=\"notFound\">\u67e5\u65e0\u5145\u503c\u8bb0\u5f55\uff0c\u8bf7\u66f4\u6539\u67e5\u8be2\u533a\u95f4\uff01</li></ul>").appendTo("#consumer_records_list");
                    $(".pageUL").hide();
                }
            }
        });
    ...
    $("#selectTime").change(function () {
        ...
        if (selectVal == "0" || selectVal == "1" || selectVal == "2" || selectVal == "3" || selectVal == "4") {
            $.ajax({
                url: "/user/ConsumeListByDeltaAjaxPageResultCount.action?pageNo=" + pageNo + "&selectTime=" + R.val() + "&userId=" + userId + "&startDate=" + startDate + "&endDate=" + endDate,
                type: "get",
                data: "String",
            ...
            var c = function () {
                ...
                $.ajax({
                    url: "/user/ConsumeListByDeltaAjaxPageResultCount.action?pageNo=" + pageNo + "&selectTime=" + R.val() + "&userId=" + userId + "&startDate=" + d + "&endDate=" + e,
                    type: "get",
                    data: "String",
                    beforeSend: changeLoading,
                    success: function (data) {
